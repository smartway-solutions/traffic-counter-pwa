...
| `#/manual` | ManualPage | 使用手冊：教學影片＋文字說明，全站唯一可垂直捲動的頁面 |
...
- 漢堡選單：匯出與統計、編輯路段／使用者（可取消）、使用手冊、更改主題、**清除本機資料**（確認 Dialog）
- 三種主題（手機優先、LocalStorage 記憶）：預設主題／夜間高對比（深色）／清單大字（單欄列表）
- 使用手冊：Playwright 錄製的 MP4 教學影片（`public/manual/`，**不納入 PWA 預快取**）
