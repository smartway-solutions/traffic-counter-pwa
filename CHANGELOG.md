...
- 修正快速交替或雙指同時按壓不同車種時，部分或全部按壓沒有計入的問題；觸控與觸控筆現在會在按下時立即計數。

## 開發者

- 時鐘移出全域 App Context，改由 Header 的獨立子元件每秒更新，避免連帶重新渲染整個路由頁面。
- 車種卡片的觸控與觸控筆輸入改由各自的 `pointerdown` 處理，並略過後續相容性 `click`；滑鼠與鍵盤仍保留原有 `click` 操作。
- 新增 Playwright E2E 測試，涵蓋快速交替、十次雙指同按，以及滑鼠與鍵盤操作的計數回歸。
